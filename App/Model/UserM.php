<?php
include_once('C:\Apache24\htdocs\Scrape_Project\App\Config\config.php');
session_start();
class User extends Database
{
  ...
  public function ModifyProfile($uid, $username, $email, $fn, $ln, $tel, $address)
  {
    try {
      $this->setConnection();
      $sql = "UPDATE `user` SET `username`=?,`email`=?,`firstName`=?,`lastName`=?,`tel`=?,`address`=? WHERE `UID` = ?";
      $stmt = $this->conn->prepare($sql);
  ...
  public function ModifyPassword($uid, $pwd)
  {
    try {
      $this->setConnection();
  ...
  public function ShowProfile($uid)
  {
    try {
      $this->setConnection();
      $sql = "SELECT username, email, firstName, lastName, tel, address FROM user WHERE `UID` = ?";
      $stmt = $this->conn->prepare($sql);
      $stmt->bindParam(1, $uid);
      $stmt->execute();
      return $stmt->fetch();
  ...
  public function DeleteAllJobs($uid)
  {
    try {
      $this->setConnection();

      $sql = "DELETE FROM job WHERE `SellerID` = ?";
      $stmt = $this->conn->prepare($sql);
      $stmt->bindParam(1, $uid);
      $stmt->execute();
      return true;
  ...
  public function DeleteAllProps($uid)
  {

    try {
      $this->setConnection();

      $sql = "DELETE FROM properties WHERE `SellerID` = ?";
      $stmt = $this->conn->prepare($sql);
      $stmt->bindParam(1, $uid);
      $stmt->execute();
      return true;
  ...
  public function DeleteAllVehc($uid)
  {
    ...
    try {
      ...
      $sql = "DELETE FROM vehicles WHERE `Seller_id` = ?";
      $stmt = $this->conn->prepare($sql);
      $stmt->bindParam(1, $uid);
      $stmt->execute();
  ...
  public function getContact($uid)
  {
    try {
      $this->setConnection();
      $sql = "SELECT `tel` FROM user WHERE `UID` = ?";
      $stmt = $this->conn->prepare($sql);
      $stmt->bindParam(1, $uid);
      $stmt->execute();
      return $stmt->fetch();
  ...
  public function SearchScrapedVehcs($q)
  {
    try {
      ...
      $sql = "SELECT * FROM `scrapedvehicles` WHERE `Title` LIKE '%$q%' OR `localisation` LIKE '%$q%' OR `type` LIKE '%$q%' ";
      $stmt = $this->conn->prepare($sql);
      $stmt->execute();
      return $stmt->fetchAll();
    } catch (PDOException $e) {
  ...
  public function SearchScrapedProps($q)
  {
    try {
      ...
      $sql = "SELECT * FROM `scrapedproperties` WHERE `Title` LIKE '%$q%' OR `localisation` LIKE '%$q%' OR `type` LIKE '%$q%' ";
      $stmt = $this->conn->prepare($sql);
      $stmt->execute();
      return $stmt->fetchAll();
    } catch (PDOException $e) {
  ...
  public function SearchScrapedJobs($q)
  {
    try {
      ...
      $sql = "SELECT * FROM `scrapedjobs` WHERE `Title` LIKE '%$q%' OR `localisation` LIKE '%$q%' OR `type` LIKE '%$q%' ";
      $stmt = $this->conn->prepare($sql);
      $stmt->execute();
      return $stmt->fetchAll();
    } catch (PDOException $e) {
